...
intensities to be expressed relative to an [\ion{O}{3}] $\lambda$5006.84 intensity of 100.
...
\cdCommand{save line intensities} command instead.} This controls details of that printout. Note that each option is intended to be used on a standalone command. If multiple options are required, they should be specified on separate lines to ensure proper execution.
...
keyword \cdCommand{air} or \cdCommand{vacuum} directly behind the wavelength.
...
This includes the \cdCommand{save continuum} family of commands which will
...
print path "foo.dat"
\end{verbatim}
which will print every instance of the file \cdFilename{foo.dat} along the
...
used as a directory separator. Multiple \cdCommand{print path} commands can be
...
in rydberg unless changed with the \cdCommand{units} option.  The third column, with
...
\cdCommand{print line vacuum} command has been used or not.
...
\cdCommand{units} option.  Note that when wavelength units are chosen, e.g.,
\cdCommand{angstrom}, the wavelengths will always be printed in vacuum regardless
of whether the \cdCommand{print line vacuum} command is used or not.  This makes
the behavior compatible with the \cdCommand{save continuum} command.
...
it understands.
Note that if you use the \cdCommand{units} option
and choose a wavelength type unit, e.g., \cdCommand{micron}, then the saved
wavelengths will always be in vacuum, regardless of what is used in the main
output. If you do not use the \cdCommand{units} option, wavelengths will be
saved in the same format as in the main output, which means they can be either
in air or vacuum, depending whether you use the \cdCommand{print line vacuum}
command.
...
and \cdFilename{O3.lin}, containing the ratio $I$(5006.84) / $I$(4363.21)
...
understood by this option. Note that if you use the \cdCommand{units} option
and choose a wavelength type unit, e.g., \cdCommand{micron}, then the saved
wavelengths will always be in vacuum, regardless of what is used in the main
output. If you do not use the \cdCommand{units} option, wavelengths will be
saved in the same format as in the main output, which means they can be either
...
If a wavelength unit is chosen (e.g., \cdCommand{micron})
...
over the bands found in \cdFilename{data/FeII\_bands.dat},
%
\begin{verbatim}
...
format of file \cdFilename{data/FeII\_bands.dat}.
...
entered.
The only exception to this rule is the \ion{Fe}{2} bands file: it is always parsed,
