...
The script \cdFilename{data/make\_data.sh} runs all the \cdCommand{compile}
commands mentioned in this Section.
It can be used to update the relevant data files.

\section{Compile stars}

Kevin Volk incorporated several large grids of stellar SEDs
into \Cloudy\ in the 1990's.
Peter van Hoof has generalized the treatment
and made several extensions.
The \cdCommand{table star} commands use these SEDs.
These commands will only function
if the atmosphere files
are installed as described on the code's web site.
This is only done once
while installing \Cloudy\ although additional SED grids
can be added later as well of course.
This step does not need to be done if
you don't want to use these stellar atmospheres.

Full instructions on how to download and install the stellar SED grids
are given at
...
Some stellar SEDs (e.g., the Rauch SEDs) need to be converted to the
\Cloudy\ internal format before they can be used.
...
This can be done with the \cdCommand{compile stars} command.
Some of these SED files are very large and it is benificial to
create index files to speed up the reading process. This is also done by
the \cdCommand{compile stars} command. This command will find all
SED grid files that are installed on the search path (including
user-defined grids) and compile them if necessary. Files that have
already been compiled on a previous occasion will be skipped.
...
and ions treated as an equivalent two-level system \citep{CloudyRevista2017}.
...
and defined in the file \cdFilename{data/continuum\_bands.dat}.
...
The wavelengths can be entered in the standard \Cloudy\ format.
Interpretation as air or vacuum wavelengths can be forced by appending the optional keyword
\cdCommand{air} or \cdCommand{vacuum} directly behind the wavelength. Note that the keyword
only applies to the wavelength directly preceding it, so it needs to be repeated, as shown below.
\begin{verbatim}
monitor Case B H-like hydrogen 0.15 range 1000A vacuum to 4.00m air
\end{verbatim}
...
\subsection{Monitor chemistry searches $<$ 3}

This performance monitor checks the convergence properties of the chemistry
solver. The quantity is the mean step length searches/chemistry step as
reported in the main output under the header ``Convergence statistics''.
This monitor will never trip in release code.

\subsection{Monitor chemistry steps $<$ 8}

This performance monitor checks the convergence properties of the chemistry
solver. The quantity is the mean steps/chemistry solve as reported in the main
output under the header ``Convergence statistics''.
This monitor will never trip in release code.
...
\subsection{Monitor itrmax $<$ 20}

This performance monitor checks the convergence properties of a calculation.
The quantity is the maximum number of calls to \cdRoutine{ConvBase} in any
given zone and iteration (including the initial search phase).
This monitor will never trip in release code.
...
\subsection{Monitor time elapsed 10 Myr [log]}

This checks the total elapsed time in a dynamical simulation of a cooling
(recombining) gas.
The same units of time are accepted as for the \cdCommand{age} command,
see Section \ref{sec:AgeCommand}.
The \cdCommand{log} option interprets the given number as the log of the
monitored age.

This command is a useful complement to the \cdCommand{stop time value}
command, Section~\ref{sec:StopTimeValue}.
...
specified on the command line, you can add an optional keyword \cdCommand{air}
or \cdCommand{vacuum} to indicate whether the specified wavelength is in air
...
will be stored at the end of the line stack and can be used like any other entry on that
stack.

If the keyword \cdCommand{quiet} appears on the \cdCommand{set blend} command
line, and any of the lines included in the blend cannot be found (e.g.,
because the element has been turned off)
the blend will be quietly skipped rather than producing error
...
\item[set chemistry TiO  [ON , OFF]]  A chemistry network 
developed by Gargi Shaw is available for testing
\citep{2024RMxAA..60..373S}. 
By default, we do not include it due to instabilities in our
linear algebra package when dealing with ill-conditioned matrices
in extreme conditions. It is well behaved for typical PDR or
protoplanetary disk conditions. 
\cdCommand{set chemistry TiO on} will enable it.
