...
\cdCommand{background}, \cdCommand{CMB}, % \cdCommand{blackbody STE},
...
% The blackbody radiation field in the STE limit
...
grids that were installed, including the custom grids that you created yourself.
This list is created by simply matching all files with names ending in \cdFilename{.ascii}.
Some basic information about each file will be printed, including the syntax of
the command needed to use that grid in a \Cloudy\ script.
The output will also list the files used by the \cdCommand{table HM05},
\cdCommand{table HM12}, and \cdCommand{table KS19} commands. They are not strictly part of the
...
grid, you can use the \cdCommand{table star "some-name.ascii" list} command. For more
